...
export async function parseTamigoExcel(file) {
  let rows
  try {
  ...
  if (hdrIdx === -1) throw new Error('Format non reconnu : colonnes de jours introuvables')
  ...
  if (dayCols.length === 0) throw new Error('Aucune date trouvée dans les en-têtes')
  ...
  return { shifts, absences, title, dates: dayCols.map(d => d.dateStr) }
}
